...
- `modules/` — miscellaneous Nix modules split by purpose:
	...
	- `copilot/copilot.nix` — GitHub Copilot CLI configuration. Skills and agents are consumed from `copilotSources` (injected via `extraSpecialArgs`) and managed by the Home Manager `programs.github-copilot-cli.skills` / `.agents` options. Instruction files from `copilotSources.instructions` are linked into `<configDir>/instructions/` where `<configDir>` is `programs.github-copilot-cli.configDir`.
	- `copilot/sources.nix` — single source of truth for all Copilot skills, agents, and instructions. Maps names to paths: local skills reference `./skills/<name>` (a directory containing `SKILL.md`), local instructions reference `./instructions/<name>.instructions.md`, and external assets reference store paths from flake inputs. Add a new external repo by: (1) declaring the input in `flake.nix` with `flake = false`, (2) destructuring it in `outputs` and passing it to `import ./modules/copilot/sources.nix { ... }`, (3) adding entries in `sources.nix`.
	...
	- `copilot/instructions/` — locally owned Copilot instruction files (for example, `conventional-commits.instructions.md`) that are deployed to the Copilot config directory during `home-manager switch`.
